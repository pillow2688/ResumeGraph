...
## 14. 生命周期收尾：范围去重与删除为什么必须联动

Phase 2 最终补丁补上了“向量生成后，资料如何长期保持正确”的问题。

`document_scope` 的作用不是页面分类，而是确定去重和未来授权边界：Profile 是所有有效
Recruiter 都能使用的候选人全局资料；Project 只能在当前 Grant 允许的项目内使用。因此不能用
一个虚拟 Project 伪装 Profile，否则未来检索会把全局资料错误地塞进项目授权集合。

精确去重按当前发布范围重算，而不是在 Chunk 创建时永久写死关系。原因是当前发布集合会随
发布、下线和删除变化。稳定规则 `(created_at, chunk_id)` 选出 canonical，其他相同 hash Chunk
标记为 `exact_duplicate` 并删除活动向量。没有 `duplicate_of_chunk_id`，所以删除某个 Chunk
不会留下需要修复的自引用链；重建只需再次查看当前事实。

`disabled_reason` 解决了一个关键歧义：`enabled=false` 可能来自秘密阻断、质量建议、管理员决定
或自动重复。如果没有来源字段，重建 canonical 时可能错误启用秘密或管理员明确禁用的内容。
当前实现只允许 `exact_duplicate` 随范围变化自动恢复。

下线与删除必须分开：

- 下线只清空当前发布指针，适合暂时停止公开，历史 Version、Chunk、Embedding 和 Job 全保留；
- 永久删除先检查活动 Job 和精确标题确认，再依赖数据库外键级联清理耐久数据；
- 两种操作之后都重建受影响范围，保证剩余 canonical 和 Embedding 与当前发布事实一致。

如果省略删除后的重建，删除旧 canonical 会让仍然存在的相同资料继续处于自动重复禁用状态，
未来检索会漏掉有效证据。若省略数据库级 cascade，只在 Python 中逐表删除，则异常中断更容易
留下孤立向量或 Job。
